...
let premiumList = [];
let contacts = {}; // { numero: {nombre, online, socket} }
...
function activatePremiumUI() {
    const accept = confirm(
        `Subir a premium cuesta 3€ (y parte de los almuerzos gratis).\nVentajas:\n` +
...
function sendMessage() {
    ...
    // Notificación emergente para usuarios premium
    if (msg.includes('mensaje_qaz') && isPremium(myNum)) {
        if (Notification.permission === 'granted') {
            new Notification(`Mensaje de ${myNum}`, { body: msg.replace('mensaje_qaz', '') });
        } else {
            Notification.requestPermission();
        }
    }

    // GIFs de Giphy si es premium
    if (msg.startsWith('https://giphy.com/') && isPremium(myNum)) {
        messagesDiv.innerHTML += `<div class="msg"><b>Tú:</b> <img src="${msg}" style="max-width:150px;"/></div>`;
    } else {
